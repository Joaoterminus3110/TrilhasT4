...
class WarehouseInventory2 extends Inventory2 {
...
class StoreInventory2 extends Inventory2 {
  ...
  addItem(item: string, quantity: number): void {
    ...
    if (novoTotal > this.LIMIT) {
      console.log(
        `⚠️ Limite excedido: "${item}" não pode ter mais de ${this.LIMIT} unidades.`
      );
